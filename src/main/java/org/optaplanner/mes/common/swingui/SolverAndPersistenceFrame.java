...
package org.optaplanner.mes.common.swingui;
...
import org.optaplanner.mes.common.business.SolutionBusiness;
...
public class SolverAndPersistenceFrame extends JFrame {
	...
	private JPanel scoreFieldPanel;
	private JTextArea scoreTextArea;
	private JPanel scoreFieldTitlePanel;
	...
	public void bestSolutionChanged() {
		...
		setScoreTextArea("Latest best score", solution);
	}

	private void setScoreTextArea(String title, Solution<?> solution) {
		Schedule schedule = (Schedule) solution;
		if (schedule != null && scoreTextArea != null) {
			Number[] scoreLevelNumbers = solution.getScore().toLevelNumbers();
			String scoreString = "";
			for (int i = 0; i != scoreLevelNumbers.length; i++) {
				if (scoreString != "") {
					scoreString += "\n";
				}
				String scoreName = schedule.getScoreDefArray()[i].getName();
				String scoreDescription = schedule.getScoreDefArray()[i].getDescription();
				String bestValueString = scoreLevelNumbers[i].toString();
	...
	private JComponent createScoreFieldPanel() {
		...
		scoreFieldPanel.add(scoreTextArea);
		...
		scoreFieldTitlePanel.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(2, 2, 2, 2),
	...
	private class TerminateSolvingEarlyAction extends AbstractAction {
		...
		public TerminateSolvingEarlyAction() {
			super("Terminate solving early", new ImageIcon(
	...
	private class SaveAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			fileChooser.setDialogTitle(NAME);
			fileChooser.setSelectedFile(new File(solutionBusiness.getSolvedDataDir(), FilenameUtils
					.getBaseName(solutionBusiness.getSolutionFileName()) + ".xml"));
	...
	private class ExportAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			fileChooser.setDialogTitle(NAME);
			fileChooser
					.setSelectedFile(new File(solutionBusiness.getExportDataDir(), FilenameUtils
							.getBaseName(solutionBusiness.getSolutionFileName())
							+ "."
	...
	private class ShowConstraintMatchesDialogAction extends AbstractAction {
		...
		public ShowConstraintMatchesDialogAction() {
			super("Constraint matches", new ImageIcon(
					SolverAndPersistenceFrame.class.getResource("showConstraintMatchesDialogAction.png")));
		}
	...
	public void resetScreen() {
		...
		setScoreTextArea("Score", solutionBusiness.getSolution());
	}
