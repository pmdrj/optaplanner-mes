...
package org.optaplanner.mes.common.persistence;
...
public abstract class AbstractTxtSolutionImporter extends AbstractSolutionImporter {
	...
	public static abstract class TxtInputBuilder extends InputBuilder {
		...
		public void readConstantLine(String constantValue) throws IOException {
			...
			if (line == null) {
				throw new IllegalArgumentException("File ends before a line is expected to be a constant value (" + constantValue + ").");
			}
		...
		public void readMesHeadingLine(String constantValue) throws IOException {
			String line = null;
		...
		public void readRegexConstantLine(String regex) throws IOException {
			String line = bufferedReader.readLine();
			if (line == null) {
				throw new IllegalArgumentException("File ends before a line is expected to be a regex (" + regex + ").");
			}
			String value = line.trim();
			if (!value.matches(regex)) {
				throw new IllegalArgumentException("Read line (" + line + ") is expected to be a regex (" + regex + ").");
			}
		}
		...
		public int readIntegerValue(String prefix, String suffix) throws IOException {
			String line = bufferedReader.readLine();
			if (line == null) {
				throw new IllegalArgumentException("File ends before a line is expected to contain an integer value (" + prefix + "<value>" + suffix + ").");
			}
		...
		public String readStringValue(String prefix, String suffix) throws IOException {
			String line = bufferedReader.readLine();
			if (line == null) {
				throw new IllegalArgumentException("File ends before a line is expected to contain an string value (" + prefix + "<value>" + suffix + ").");
			}
		...
		public String[] splitBy(String line, String tokenRegex, String tokenName, Integer minimumNumberOfTokens, Integer maximumNumberOfTokens, boolean trim,
			...
			if (minimumNumberOfTokens != null && lineTokens.length < minimumNumberOfTokens) {
				throw new IllegalArgumentException("Read line (" + line + ") has " + lineTokens.length + " tokens but is expected to contain at least "
						+ minimumNumberOfTokens + " tokens separated by " + tokenName + ".");
			}
			if (maximumNumberOfTokens != null && lineTokens.length > maximumNumberOfTokens) {
				throw new IllegalArgumentException("Read line (" + line + ") has " + lineTokens.length + " tokens but is expected to contain at most "
						+ maximumNumberOfTokens + " tokens separated by " + tokenName + ".");
			}
			if (trim) {
